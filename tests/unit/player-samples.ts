...
export const STATS = {
  ...
  data: {
    ...
    mvpc: 73,
    wsc: 11,
    mo: { v: 112848, ts: 1726010389, hid: 36, bid: 4110381620662451700, sid: 0, hid_e: entity(36), bid_s: "4110381620662451526" },
    sids: [40, 39, 38, 37],
  },
};
...
export const FREQUENT_HEROES = {
  ...
  data: {
    ...
    result: [
      { hid: 84, tc: 25, wc: 17, bs: 902.5, mr: 7000, mrp: 0.7, hid_e: entity(84), p: 1600 },
      { hid: 20, tc: 12, wc: 4, bs: 650, mr: 5000, mrp: 0.4, hid_e: entity(20), p: 1200 },
      { hid: 17, tc: 8, wc: 7, bs: 844.875, mr: 6626, mrp: 0.6188, hid_e: entity(17), p: 1460 },
      { hid: 36, tc: 6, wc: 2, bs: 700, mr: 4000, mrp: 0.3, hid_e: entity(36), p: 1100 },
      { hid: 65, tc: 3, wc: 1, bs: 600, mr: 3000, mrp: 0.2, hid_e: entity(65), p: 1000 },
      { hid: null, tc: 5, wc: 5, hid_e: null },
      { hid: 31, tc: 0, wc: 0, hid_e: entity(31) },
      "n'importe quoi",
    ...
    result: [
      {
        ...
        hid_e: entity(31), its_e: [{ id: 2305, n: "Swift Boots", ix: "", i2x: "" }, null], hlvl: 15, rname: "Jungle",
      },
    ],
  },
};
...
export function pageHistory(matches: object[], next: string | null) {
  ...
  const text = JSON.stringify({ code: 0, message: "Success", data: { pageInfo, result: matches } });
  return text.replace(/"nextCursor":"(\d+)"/, '"nextCursor":$1');
}
...
export function historyRaw() {
  const timer = STORY.split("").map((c, i) =>
    matchRaw(i, [84, 20, 17, 36][i % 4], [4, 3, 4, 2][i % 4], c === "V" ? 1 : 0, START_STORY + i * 6 * 3600),
  );
  timer.splice(20, 0, matchRaw(900, 84, 4, null, START_STORY + 19 * 6 * 3600 + 60));
  timer[5].ts = null;
...
export function detailMatch(myHero: number, enemies: number[], win: boolean, withTeams = true) {
  const player = (hid: number, f: number | null, rid: number, won: boolean) => ({
    ...
    ts: 1774857999, bd: 1200, fk: 20, fw: won ? 1 : 0, hid_e: entity(hid), its_e: [null], hlvl: 15, rname: "x",
  });
  ...
  return {
    ...
    data: {
      result: [
        player(myHero, withTeams ? 1 : null, ME.roleId, win),
        ...allies.map((h, i) => player(h, withTeams ? 1 : null, 1000 + i, win)),
        ...enemies.map((h, i) => player(h, withTeams ? 2 : null, 2000 + i, !win)),
      ],
    },
  };
}
