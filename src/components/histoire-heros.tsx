...
function GroupeChips({
  ...
  return (
    <div>
      <dt className="flex items-center gap-1.5 text-xs uppercase tracking-wide text-craie-500">
      ...
      <dd className="mt-2 flex flex-wrap gap-1.5">
        {valeurs.map((v) => (
          <span
            key={v}
            className="biseau-sm border border-nuit-700/70 bg-nuit-800/60 px-2 py-1 text-xs text-craie-200"
          >
            {v}
          </span>
        ))}
  ...
  return (
    <div className="space-y-12">
      ...
      {accroche && (
        <figure className="relative overflow-hidden">
          <Quote
            aria-hidden
            size={80}
            className="pointer-events-none absolute -left-3 -top-4 text-nuit-800"
            strokeWidth={1.5}
          />
          <blockquote className="relative pl-6 font-titre text-xl italic leading-relaxed text-craie-100 sm:text-2xl">
            {accroche}
          </blockquote>
        </figure>
      )}
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start">
          ...
          {lore.length > 0 ? (
            <section>
              ...
              <div className="mt-5 max-w-[68ch] space-y-4 text-[0.95rem] leading-[1.8] text-craie-300">
                {premier && (
                  <p className="first-letter:float-left first-letter:mr-3 first-letter:mt-1 first-letter:font-titre first-letter:text-5xl first-letter:font-bold first-letter:leading-[0.8] first-letter:text-or-400">
                    {premier}
                  </p>
                )}
                {suite.map((p, i) => (
        ...
        {/* La fiche d'identite. */}
        {fiche && (
          <aside className="biseau border border-nuit-700/60 bg-nuit-950/50 p-5 lg:sticky lg:top-24">
            <h2 className="font-titre text-sm font-bold uppercase tracking-wide text-craie-100">
        ...
        <section className="border-t border-nuit-800 pt-10">
          ...
          <ul className="mt-5 grid gap-3 sm:grid-cols-2">
            {anecdotes.map((a, i) => (
              <li
                key={i}
                className="biseau-sm border border-nuit-700/50 bg-nuit-900/40 p-4 text-sm leading-relaxed text-craie-300"
              >
                {a}
              </li>
            ))}
